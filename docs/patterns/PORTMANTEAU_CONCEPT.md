...
**Result:** 
- ðŸ’¥ Tool list overwhelming for users
- ðŸ¤¯ Cognitive overload in Claude Desktop
- ðŸ“Š Difficult to discover related operations
- ðŸ” Hard to understand what's available
- ðŸ“± Poor mobile/UI experience
...
**Result:** 60+ tools â†’ 6-7 clean portmanteau tools (with same 60+ operations)
...
### âŒ Don't Use Portmanteau When:
...
â”œâ”€â”€ portmanteau/
â”‚   â”œâ”€â”€ __init__.py                    # Registers all portmanteau tools
â”‚   â”œâ”€â”€ vm_management.py               # VM lifecycle (10 ops)
â”‚   â”œâ”€â”€ network_management.py          # Networks (5 ops)
â”‚   â”œâ”€â”€ snapshot_management.py         # Snapshots (4 ops)
â”‚   â”œâ”€â”€ storage_management.py          # Storage (6 ops)
â”‚   â”œâ”€â”€ system_management.py           # System info (5 ops)
â”‚   â”œâ”€â”€ discovery_management.py        # Help/info (4 ops)
â”‚   â””â”€â”€ hyperv_management.py           # Hyper-V (4 ops, Windows)
â”œâ”€â”€ vm/
â”‚   â””â”€â”€ vm_tools.py                    # Individual VM functions
â”œâ”€â”€ network/
â”‚   â””â”€â”€ network_tools.py               # Individual network functions
â”œâ”€â”€ snapshot/
â”‚   â””â”€â”€ snapshot_tools.py              # Individual snapshot functions
â””â”€â”€ register_tools.py                  # Tool mode selector
```
...
async def vm_management(
    ...
    if action == "start":
        return await start_vm(vm_name=vm_name)  # â† Delegates to individual tool
```
...
## FastMCP 3.1.1+ Compliance Requirements

### 1. Self-Registration Pattern âœ…
...
def register_category_management_tool(mcp: FastMCP) -> None:
    ...
    @mcp.tool()  # â† No description parameter!
    async def category_management(...):
...
**Why:**
- FastMCP 3.1.1+ extracts descriptions from docstrings
- Explicit `description` parameter OVERRIDES docstring
- Self-registration pattern keeps tools modular

### 2. Literal Types for Discoverability âœ…

**Critical:**
```python
async def category_management(
    action: Literal["op1", "op2", "op3"],  # â† MUST have Literal!
    ...
):
```
...
### 4. No Description Parameter âŒ

**Wrong:**
```python
@mcp.tool(description="Manages categories")  # âŒ Overrides docstring!
```

**Right:**
```python
@mcp.tool()  # âœ… Uses docstring automatically
```

**Critical:** FastMCP 3.1.1+ uses docstrings UNLESS you provide explicit description parameter which then OVERRIDES everything!
...
def register_{category}_management_tool(mcp: FastMCP) -> None:
    ...
    async def {category}_management(
        action: Literal["operation1", "operation2", ...],  # â† MUST have Literal!
        # ... parameters ...
    ) -> dict[str, Any]:
        '''
...
```bash
# Check imports
python -c "from virtualization_mcp.tools.portmanteau.new_portmanteau import register_{category}_management_tool; print('âœ… Imports OK')"
...
### âŒ Trap 1: Forgetting Literal Type

**Problem:**
```python
async def category_management(action: str, ...):  # âŒ
```
...
**Fix:**
```python
async def category_management(action: Literal["op1", "op2"], ...):  # âœ…
```

### âŒ Trap 2: Using description Parameter

**Problem:**
```python
@mcp.tool(description="Manages categories")  # âŒ Overrides docstring!
async def category_management(...):
    '''Detailed docstring with all operations...'''  # â† Never seen by Claude!
```
...
@mcp.tool()  # âœ… Uses docstring automatically
async def category_management(...):
    '''Detailed docstring...'''  # â† Claude sees this!
```

### âŒ Trap 3: Circular Import in Discovery/Help Tools

**Problem:**
```python
# In discovery portmanteau
from virtualization_mcp.all_tools_server import mcp  # âŒ Not exported!
```
...
### âŒ Trap 4: Incomplete Docstrings
...
### âŒ Trap 5: Literal Out of Sync with Implementation

**Problem:**
```python
action: Literal["op1", "op2"]  # â† Only 2 listed

# But implementation has:
if action == "op1": ...
elif action == "op2": ...
elif action == "op3": ...  # âŒ Missing from Literal!
```
...
### âŒ Trap 6: Missing Error Handling
...
### âŒ Trap 7: Forgetting to Run Ruff
...
1. **Add to ACTIONS dict:**
   ...
   ACTIONS = {
       "existing_op": "...",
       "new_op": "New operation description",  # â† Add here
   }
   ```

2. **Add to Literal type:**
   ```python
   action: Literal["existing_op", "new_op"]  # â† Add here
   ```
...
âœ… **File header docstring** explaining what it consolidates  
âœ… **ACTIONS dict** for validation and error messages  
âœ… **Literal type** with all operations enumerated  
âœ… **@mcp.tool()** with NO description parameter  
âœ… **Comprehensive function docstring** with:
  - Purpose and scope
...
âœ… **Validation** of action against ACTIONS dict  
âœ… **Routing logic** for each action  
âœ… **Error handling** with helpful error messages  
âœ… **Private handler functions** (_handle_*) for each action  
âœ… **No import errors** - test imports work  
âœ… **Ruff clean** - zero linting errors  
...
### For Users:
- ðŸŽ¯ Clean, organized tool list (5-10 vs 60+)
- ðŸ“š Easier discovery and learning
- ðŸ” Logical grouping by category
- ðŸ’¡ Better understanding of capabilities
- âš¡ Faster to find what they need

### For Developers:
- ðŸ—ï¸ Better code organization
- ðŸ“¦ Modular registration system
- ðŸ”§ Switchable modes (production/testing)
- ðŸ§ª Individual tools still available
- ðŸ“– Self-documenting architecture

### For MCP Clients (Claude):
- ðŸš€ Discovers all operations at startup
- ðŸ“‹ Sees enumerated actions in schema
- ðŸ’¬ Can explain specific capabilities
- âœ… Knows parameter requirements
- ðŸŽ¨ Better suggestions and routing
...
1. âœ… Use `Literal` types for action enums (discoverability)
2. âœ… Use `@mcp.tool()` without description (FastMCP 3.1.1+)
3. âœ… Write comprehensive docstrings (document all operations)
4. âœ… Validate imports (no circular dependencies)
5. âœ… Run ruff (code quality)
6. âœ… Test registration (works without errors)
7. âœ… Verify in Claude Desktop (actual usage)
...
virtualization-mcp: 60+ tools â†’ 6-7 portmanteau tools (same functionality, 10x cleaner UX)

---

**Status:** Production-ready pattern, proven in virtualization-mcp âœ¨
