...
| Repo ships Tauri native installer | Bundle rules in this doc + [tauri_nsis_building.md](./rules/tauri_nsis_building.md) |
...
See [tauri_nsis_building.md](./rules/tauri_nsis_building.md) for build layout.

**Release artifact:** one NSIS `{Product}_{version}_x64-setup.exe`. After install, user sees **one app shortcut** — Python backend is embedded inside the operator bundle, not a second exe in the install folder.

### Include in the single installer

| Component | Yes |
|-----------|-----|
| Python MCP server + FastMCP stack (embedded `resources/*.exe`) | Yes |
| React/Vite `webapp/dist/` static assets | Yes |
| Tauri operator shell (WebView2 host) | Yes |
| Repo-owned prompts, tiny bridge assets, optional MCP registration script | Yes |
...
| [rules/tauri_nsis_building.md](./rules/tauri_nsis_building.md) | Single NSIS installer, embedded PyInstaller backend |
