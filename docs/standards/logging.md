...
> **Fleet note:** The content below covers generic Python logging configuration. The fleet uses
> two additional standardized patterns that MUST be implemented per-repo:
>
> 1. **Ring-buffer logging backend** — each Tauri-wrapped repo exposes a REST endpoint
>    (`GET /api/v1/logs`) backed by a fixed-size deque or SQLite ring buffer, viewable in the
>    webapp's Logs page. Reference repos: `tailscale-mcp`, `inkscape-mcp`, `fastsearch-mcp`.
>    See [WEBAPP_LOGS_PAGE.md](./WEBAPP_LOGS_PAGE.md).
>
> 2. **`backend-spawn.log`** — the Tauri Rust shell writes backend lifecycle logs to
>    `%LOCALAPPDATA%\{identifier}\cache\backend-spawn.log` for diagnosing startup failures
>    (see [rules/tauri_nsis_building.md](./rules/tauri_nsis_building.md) `backend.rs`).
