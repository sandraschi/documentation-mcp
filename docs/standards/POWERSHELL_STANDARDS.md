...
## 6. Stderr Redirection: `2>$null`, NOT `2>nul` (CRITICAL FOOTGUN)

**NEVER use `2>nul` in PowerShell.** This is the single most common PowerShell footgun in the fleet.

| Syntax | What it does | In cmd.exe | In PowerShell |
|--------|-------------|------------|--------------|
| `2>nul` | Redirect stderr to null | ✅ Works (NUL device) | ❌ Creates a file named `nul` in CWD |
| `2>$null` | Redirect stderr to null | ❌ Syntax error | ✅ Works (PowerShell `$null`) |

In cmd.exe, `2>nul` redirects stderr to the NUL device. PowerShell inherited the `2>` syntax but interprets `nul` as a **filename**, creating a literal file called `nul` in the current directory. This is especially destructive during NSIS installs (creates `nul` in `$INSTDIR`) and anywhere the CWD is not the repo root (creates orphan files).

### The fix

Always `2>$null` in PowerShell. The `$null` automatic variable represents the null value — redirecting to it discards the stream:

```powershell
# WRONG — creates file called "nul"
taskkill /F /IM myapp.exe /T 2>nul

# CORRECT — discards to null
taskkill /F /IM myapp.exe /T 2>$null

# ALSO CORRECT — cleaner, same result
taskkill /F /IM myapp.exe /T -ErrorAction SilentlyContinue
```

### Escape for subprocess calls

When passing `2>$null` inside a command string to `Start-Process` or `subprocess.run`:

```powershell
# PowerShell via Start-Process (escape $null with backtick or single quotes)
Start-Process powershell -ArgumentList '-NoProfile -Command "taskkill /F /IM myapp.exe /T 2>$null"'

# Python subprocess (f-string — $ is not special)
subprocess.run(["powershell", "-NoProfile", "-Command",
    "taskkill /F /IM myapp.exe /T 2>$null"])

# Rust Command (format! — $ is not special)
...
The fleet includes a scanner for this footgun:
```powershell
powershell -NoProfile -File D:\Dev\repos\mcp-central-docs\scripts\check-redirect-safety.ps1 -RepoPath .
```
