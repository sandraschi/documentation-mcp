...
status: obsolete
superseded_by: standards/JUNE_2026_STANDARDS_BAR.md + standards/AGENTS.md
last_updated: 2026-03-30
obsolete_reason: >
  References FastMCP 3.1.1+ (floor is now 3.2+) and explicitly rejects the
  portmanteau pattern, which is current fleet SOTA. Do not use for guidance.
  Kept for historical reference only.
---

> [!WARNING]
> **OBSOLETE — do not follow.** This doc contradicts current fleet standards:
> it sets FastMCP 3.1.1+ as the floor (current: 3.2+) and advises abandoning
> the portmanteau pattern (current: mandatory). See `JUNE_2026_STANDARDS_BAR.md`.
...
- [ ] Root `pyproject.toml` with `fastmcp>=3.4.4,<4`.
