...
| CORS | `allow_origin_regex` **unconditionally** covering Tailscale `*.ts.net`, LAN IPs, CGNAT, Tauri, and localhost — per `CORS_STANDARD.md` | `allow_origin_regex` only when `{REPO}_TAURI=1`, no LAN/Tailscale coverage, `allow_origins=["*"]` |
