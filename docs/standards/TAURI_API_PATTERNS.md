...
> Every import is guarded by an `isTauri()` gate so browser mode never tries to load Tauri APIs.
> See [BUG-007](../troubleshooting/details/BUG-007_Tauri_Zoom_Dev_Broken.md) for the antipattern that `isTauri()` solves.
>
...
### 3. Safe branch pattern (always)

Check `isTauri()` before importing Tauri modules. This avoids a wasted ~50KB
dynamic module load in the browser and makes the two code paths explicit:
...
if (isTauri()) {
  ...
  // Browser fallback (same result, different API)
}
```

**Never** use bare try/catch around Tauri dynamic imports in place of a proper branch:

```typescript
// WRONG — empty catch rots, main path looks secondary:
try {
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("my_command");
} catch {
  // browser fallback — but which branch is the intent?
}

// RIGHT — both branches are equal citizens:
if (isTauri()) {
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("my_command");
} else {
...
Also never use static imports for Tauri modules:

```typescript
// WRONG — crashes dev browser at module resolution:
import { invoke } from "@tauri-apps/api/core";

// RIGHT — dynamic after isTauri() guard:
const { invoke } = await import("@tauri-apps/api/core");
```
