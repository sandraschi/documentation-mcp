...
```python
# âŒ WRONG - Empty catch block
try:
    risky_operation()
except:
    pass  # Silent failure - NOT ALLOWED

# âŒ WRONG - Bare except
try:
    risky_operation()
except Exception:
    logger.error("Something went wrong")  # Too vague

# âœ… CORRECT - Specific handling
try:
    risky_operation()
...
> [!IMPORTANT]
> **FastMCP Parameter Naming**: When raising `MCPError`, always use `message=` instead of `description=`. FastMCP 3.1+ (and 3.1.1+.1+) does not recognize `description` and will lose the error context.
