...
> **Note:** This file describes **creative workflow sampling** — iterating over design/3D/audio variations
> in wrapper MCPs (Blender, GIMP, etc.). For FastMCP's built-in `ctx.sample()` (LLM calling back to the
> connected host within a tool), see `SOTA_REQUIREMENTS.md` §2.1 and `TOOL_DESIGN_STANDARDS.md` §2.1.
...
## See Also
- [SOTA_REQUIREMENTS.md](./SOTA_REQUIREMENTS.md) §2.1 — FastMCP 3.4+ `ctx.sample()` for LLM-invoked tool calls
- [TOOL_DESIGN_STANDARDS.md](./TOOL_DESIGN_STANDARDS.md) §2.1 — Sampling within tool implementations
