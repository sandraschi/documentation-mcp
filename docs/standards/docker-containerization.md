...
## Overview
Standards for containerizing MCP servers using Docker.

> **Fleet note:** Docker is used for **infrastructure MCPs** (monitoring, Weaviate, traefik) and
> CI/CD only. **Primary distribution** for end-user desktop MCPs is **PyInstaller + Tauri NSIS**
> (see [rules/tauri_nsis_building.md](./rules/tauri_nsis_building.md)). Do not Dockerize servers
> that ship as NSIS installers.
...
# Install dependencies (fleet standard: uv, not pip)
...
# Install MCP server (fleet standard: uv)
COPY pyproject.toml uv.lock .
RUN uv sync --no-dev
...
CMD ["uv", "run", "python", "-m", "creative_mcp.server"]
```
...
# Bind to a fleet-range port (see WEBAPP_PORTS.md)
MCP_PORT=10700
...
```python
# Environment-based configuration (Pydantic v2)
...
## Docker Desktop Daemon Reliability

Docker Desktop on Windows (WSL2 backend) is prone to daemon non-responsiveness during
compose operations — especially `build --no-cache` on large images. This is a known
fleet issue (see [DOCKER_WINDOWS_RESILIENCE.md](../docker/DOCKER_WINDOWS_RESILIENCE.md)).

### Pre-build health check

Before any `docker compose build` or `docker compose up -d`, verify the daemon is
actually responsive:

```powershell
# Pre-flight: confirm daemon isn't in zombie state
docker info 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Host "DOCKER DAEMON UNRESPONSIVE — do not proceed." -ForegroundColor Red
    Write-Host "Recovery:"
    Write-Host "  1. 'taskkill /F /IM \"Docker Desktop.exe\" /T'"
    Write-Host "  2. Restart Docker Desktop, wait 60s"
    Write-Host "  3. 'wsl --terminate docker-desktop' if still stuck"
    exit 1
}
```

### What to do when the daemon hangs mid-build

```powershell
# Triple Kill (see monitoring/README.md)
...
Start-Sleep 90  # daemon needs time to init

# Resume build (build cache is lost)
docker compose build --no-cache
```

### When to skip Docker and use uv directly

If Docker Desktop is repeatedly unreliable for a given repo, skip containers and
...
This bypasses Docker entirely. Most fleet MCP servers are designed for dual
transport — Docker is optional infrastructure for monitoring sidecars, not
