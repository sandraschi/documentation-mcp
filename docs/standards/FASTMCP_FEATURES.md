...
mcp_http = mcp.http_app(path="/")  # path="/" is required when mounting
app = FastAPI()
app.mount("/mcp", mcp_http)
# Note: http_app defaults to path="/mcp" which creates a double-prefix
# when mounted ("/mcp" mount strips the prefix, but the internal route
# still expects "/mcp"). Always use path="/" when the app will be mounted.
```
