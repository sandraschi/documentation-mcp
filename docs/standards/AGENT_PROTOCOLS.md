...
- **New MCP server scaffolding:** Cursor rule **`.cursor/rules/new-mcp-server-questionnaire.mdc`** — when creating a new `*-mcp` repo, run the **pre-flight questionnaire** (or accept “defaults”) before writing files; ports → [WEBAPP_PORTS.md](../operations/WEBAPP_PORTS.md). **Target:** [NEW_REPO_BUILD_COMPLETE.md](./NEW_REPO_BUILD_COMPLETE.md) — first `assfix` should find nothing CRITICAL/HIGH.
- **[New-repo build complete (assfix-zero)](./NEW_REPO_BUILD_COMPLETE.md)**: Initial scaffold must pass the assess rubric day one; assfix is not the cleanup crew for runt builds.
...
- **[Onboarding Standard](./ONBOARDING_STANDARD.md)**: **DEFAULT on new-repo gate** — wrappees must be installed/runnable for joy (even Notepad++). Ship `docs/ONBOARDING.md` (what for, money/CC, pitfalls), **big red under-hero CTA**, **MOCK-until-onboarded** UI that clears when configured. N/A only for **rare: no wrappee and no online account necessary**.
