...
### Strict JSON-RPC Implementation (Windows/Antigravity IDE)

When running in **Stdio** mode, especially on Windows or with strict JSON-RPC parsers like the **Antigravity IDE**, the server must ensure the stdout stream remains perfectly clean. Any unexpected output (logs, banners, `\r` characters) will corrupt the stream and break the connection.

#### 1. Binary Mode Enforcement
On Windows, standard I/O defaults to "text mode", which automatically converts `\n` to `\r\n`. Strict parsers like Antigravity IDE reject `\r\n` as invalid trailing data.
...
#### 2. Output Suppression (`DevNullStdout`)
During initialization, libraries may print to `stdout`. To prevent this, temporary redirection to a "null" object is required.

```python
class DevNullStdout:
    """Discards all writes to prevent stream pollution."""
...
#### 3. Strict LF Wrapper (`ForceLFStdout`)
Even with binary mode, some libraries might explicitly write `\r\n`. A wrapper ensures only `\n` passes through.

```python
class ForceLFStdout:
    """Wraps stdout to ensure only LF (\n) line endings are used."""
    def __init__(self, original): self.original = original
    def write(self, s): return self.original.write(s.replace("\r\n", "\n"))
    def flush(self): self.original.flush()

# Before mcp.run():
sys.stdout = ForceLFStdout(original_stdout)
```

#### 4. Logger Neutralization
Standard logging must be disabled or redirected to `stderr`.

```python
import logging
from loguru import logger

# Redirect all standard logging to stderr
logging.basicConfig(level=logging.INFO, stream=sys.stderr)

# For stdio mode, neutralize noisy loggers
if is_stdio_mode:
    logger.remove() # Remove default loguru handler
    # Optional: Use a NoOpLogger class for absolute silence
```
