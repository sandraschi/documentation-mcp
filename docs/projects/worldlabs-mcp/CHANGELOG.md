...
- FastMCP 3.1.1++ integration for Claude Desktop
- `generate_world_from_text` â€” text prompt to 3D world
- `generate_world_from_image` â€” public image URL to 3D world (with panorama support)
- `generate_world_from_multi_image` â€” multiple images at azimuth angles
- `generate_world_from_video` â€” public video URL to 3D world
- `upload_and_generate` â€” local file upload + generation (end-to-end)
- `prepare_media_upload` â€” signed GCS URL for manual file upload
- `generate_world_from_media_asset` â€” generate from uploaded asset
- `get_operation` â€” single status poll
- `wait_for_world` â€” blocking poll with error detection and timeout
- `get_world` â€” fetch world details and asset URLs
- `list_worlds` â€” paginated world listing
