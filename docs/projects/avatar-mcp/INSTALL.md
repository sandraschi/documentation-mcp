...
4. Start the server:
   ...
   uv run python -m avatar_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn avatar_mcp.server:app --port 10793
   ```
5. Open `http://localhost:10793` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/avatar-mcp/issues) |
