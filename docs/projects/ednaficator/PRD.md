...
**Status**: Runt project â†’ Revival with focused MVP scope  
...
Ednaficator is **not** a standalone AI service. It's a **conversational MCP orchestrator** â€” an intermediary layer that lets family members talk naturally to the existing MCP infrastructure without needing to understand tool APIs, configuration, or technical details.
...
Sandra: "I'll SSH into Goliath, load Claude Desktop, 
         check devices-mcp... it's online."
```

**With Ednaficator**: Family members ask directly.
```
Marion: "Is Benny's camera online?"
Ednaficator: Loads devices-mcp from Claude Desktop config,
...
EdnaCore Orchestrator
  â”œâ†’ Claude Desktop config.json (MCP registry)
  â”œâ†’ Ollama (local LLM inference)
  â”œâ†’ 20+ MCP Servers (stdio)
  â””â†’ Advanced Memory MCP (persistent context)
```
...
|-----------|--------|---------|
| `api_bridge.py` | âœ… Exists (346 lines) | FastAPI + WebSocket server |
| `EdnaCore` | ðŸ†• Build | LLM orchestrator, tool calling |
| `MCP Loader` | ðŸ†• Build | Parse config.json, discover tools |
| `Tool Executor` | ðŸ†• Build | Call MCPs via stdio |
| `Memory Bridge` | ðŸ†• Build | Interface with Advanced Memory MCP |
| `Vienna Context` | ðŸ†• Build | Weather, transit, holidays, districts |

### V1 MCP Servers (Integrated)

**Already Available** (load from Claude Desktop config):
- `devices-mcp` - Camera status, recording
- `homecontrol-mcp` - Smart home automation
- `local-llm-mcp` - LLM inference fallback
- `advanced-memory-mcp` - Context/memory storage
- **`email-mcp` (minimail-mcp)** - Send/receive email âœ¨ **NEW**
...
EdnaCore executes:
  1. Loads homecontrol-mcp + devices-mcp
  2. LLM chains:
...
EdnaCore executes:
  1. Loads homecontrol-mcp + devices-mcp + other monitoring MCPs
...
Response: "Weather looks good tomorrow (18Â°C, sunny). 
...
1. **React UI** â†’ WebSocket: `{"message": "Is Benny camera online?"}`
2. **api_bridge.py** receives, passes to EdnaCore
3. **EdnaCore**:
   - Loads Claude Desktop config.json
   - Extracts devices-mcp tools
   - Creates LLM context: "Available tools: [devices-mcp tools list]"
   - Calls Ollama with prompt + context
4. **Ollama** (local inference on 4090):
   - Generates: `{"tool": "devices-mcp", "action": "get_camera_status", "args": {"name": "benny-cam"}}`
5. **Tool Executor** calls devices-mcp via stdio
6. **devices-mcp** returns: `{"online": true, "recording": true, "battery": 95}`
...
9. **api_bridge.py** â†’ WebSocket â†’ **React UI** displays response

### Data Flow: "Send reminder email to Marion"

1. **React UI** â†’ WebSocket: `{"message": "Remind Marion about Benny's vet appointment"}`
2. **api_bridge.py** â†’ **EdnaCore**
3. **EdnaCore**:
...
10. **api_bridge.py** â†’ WebSocket â†’ **React UI** displays confirmation
...
âŒ Austrian government APIs (Wien.gv.at, Ã–BB, FinanzOnline)  
âŒ OAuth/credential management (use stored config only)  
âŒ Advanced UI features (minimal React, focus backend)  
âŒ Multi-user support (single user, local network)  
âŒ Mobile app / Voice I/O  
âŒ Calendar automation (email notifications yes, calendar integration no)  
âŒ Slack/Discord bot integrations (email-mcp supports them but not orchestrated yet)  
...
- [ ] Implement tool calling (parse LLM output â†’ MCP calls)
...
| Memory Integration | âœ… | Can read/write/search context |
...
| **email-mcp (minimail-mcp)** | Pre-built, multi-service, FastMCP 3.1.1+.3 compliant, zero dev cost | Already production-ready in D:\Dev\repos\email-mcp |
...
| **GTFS over Ã–BB API** | Free, no credentials needed | Limited real-time accuracy (acceptable for MVP) |
...
A: Cost (â‚¬0 vs â‚¬2-5/day), privacy (local), latency (<500ms on 4090 vs 3-5s API). For non-critical tasks, local is superior.
