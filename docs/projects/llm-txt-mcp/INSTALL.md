...
4. Start the server:
   ...
   uv run python -m llm_txt_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn llm_txt_mcp.server:app --port 10837
   ```
5. Open `http://localhost:10837` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/llm-txt-mcp/issues) |
