...
4. Start the server:
   ...
   uv run python -m mcp_federation_hub.server

   # HTTP mode (for web dashboard)
   uv run uvicorn mcp_federation_hub.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/mcp-federation-hub/issues) |
