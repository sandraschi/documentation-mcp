...
git clone https://github.com/sandraschi/wall-flower
cd wall-flower
just
```
...
4. Start the server:
   ...
   uv run python -m wall_flower.server

   # HTTP mode (for web dashboard)
   uv run uvicorn wall_flower.server:app --port 10984
   ```
5. Open `http://localhost:10984` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/wall-flower/issues) |
