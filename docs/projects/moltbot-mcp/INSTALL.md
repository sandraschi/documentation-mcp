...
4. Start the server:
   ...
   uv run python -m moltbot_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn moltbot_mcp.server:app --port 10730
   ```
...
5. Open `http://localhost:10730` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/moltbot-mcp/issues) |
