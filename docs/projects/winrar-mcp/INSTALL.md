...
4. Start the server:
   ...
   uv run python -m winrar_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn winrar_mcp.server:app --port 10762
   ```
...
5. Open `http://localhost:10762` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/winrar-mcp/issues) |
