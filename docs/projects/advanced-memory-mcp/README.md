...
- **Portmanteau Tools**:
    - **adn_knowledge_rag**: High-density context retrieval bridge for robofang.
- **Document Intelligence**: PDF/EPUB processing with intelligent chunking and automated vector ingestion.
- **Agentic Workflows**: FastMCP 3.1.1+.1+ SEP-1577 sampling with real tool functions — LLM autonomously orchestrates multi-step knowledge workflows via `ctx.sample(tools=[...], result_type=Model)`
