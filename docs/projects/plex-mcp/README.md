...
[![Status](https://img.shields.io/badge/Status-Production%20Ready-brightgreen)](https://github.com/yourusername/plex-mcp)

> **âš ï¸ PRODUCTION READY**: PlexMCP v3.2.0 is production-ready with FastMCP 3.2 compatibility, comprehensive error handling, and monitoring capabilities. **Playback control (`plex play`, `plex pause`) is currently non-functional for ALL clients**, including GDM-discoverable clients (PlexAmp) and non-GDM clients (Plex Web, Plex for Windows). See [STATUS.md](STATUS.md) for detailed status.

A FastMCP 3.2 server implementation for managing Plex Media Server with a clean, type-safe API and production-ready features.

## âœ¨ Features

- **FastMCP 3.2 Compatibility**: Full support for latest FastMCP features and universal connect patterns
- **Enhanced Error Handling**: Comprehensive error handling with structured responses and proper logging
- **Production Monitoring**: Built-in health checks, metrics collection, and monitoring endpoints
- **Deep Media RAG**: Recursive traversal for episodes and albums with hierarchical context (Show -> Season -> Episode)
- **Industrial Maintenance**: High-fidelity dashboard for library health, real-time sync telemetry, and maintenance control
- **Neural Media RAG**: High-speed LanceDB semantic retrieval via the `plex_rag` unified tool
...
### **Portmanteau Pattern Benefits** ðŸŽ¯

- **Reduced Tool Count**: 52+ individual tools â†’ 15 logical categories (71% reduction)
...
- **FastMCP 3.1.1++ Compliant**: Uses Literal types for operation parameters
- **AI-Friendly**: Comprehensive docstrings with structured error responses

---

## ðŸš€ MCPB Package Features
