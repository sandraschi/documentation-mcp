...
4. Start the server:
   ...
   uv run python -m multi_backup_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn multi_backup_mcp.server:app --port 10799
   ```
5. Open `http://localhost:10799` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/multi-backup-mcp/issues) |
