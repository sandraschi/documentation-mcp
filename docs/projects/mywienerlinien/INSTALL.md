...
4. Start the server:
   ...
   uv run python -m mywienerlinien.server

   # HTTP mode (for web dashboard)
   uv run uvicorn mywienerlinien.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/mywienerlinien/issues) |
