...
4. Start the server:
   ...
   uv run python -m chitchat.server

   # HTTP mode (for web dashboard)
   uv run uvicorn chitchat.server:app --port 10974
   ```
5. Open `http://localhost:10974` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/chitchat/issues) |
