...
A **FastMCP 3.1.1+ compliant** MCP server that enables natural language control of professional audio/visual applications through the **Open Sound Control (OSC)** protocol. Control Ableton Live, TouchDesigner, VRChat, Max/MSP, and other OSC-enabled applications directly from Claude Desktop or any MCP client.

> **âš ï¸ Beta Status:** This project is currently in **beta**. Core functionality is stable and production-ready, but some advanced features may change. We welcome feedback and contributions!
...
- ðŸŽµ **DAW Control**: Automate Ableton Live, Logic Pro, and other music production software
- ðŸŽ¨ **Visual Programming**: Control TouchDesigner, Resolume Arena, and VJ software
- ðŸŽ® **VR/Gaming**: Manipulate VRChat avatars and game parameters
- ðŸ”Š **Audio Synthesis**: Program SuperCollider, Max/MSP, and Pure Data
- ðŸŽ›ï¸ **Hardware Control**: Interface with MIDI controllers and modular synths (VCV Rack)
- ðŸŒ **Creative Coding**: Integrate with Processing, openFrameworks, and other platforms

## ðŸ”„ Bidirectional OSC Communication
...
### Core Capabilities
- âœ… **FastMCP 3.1.1+ Compliant** - Latest protocol support with server lifespans and caching
- âœ… **Bidirectional Communication** - Send and receive OSC messages
- âœ… **Response Caching** - 60-second TTL for improved performance
- âœ… **Input Validation** - Pydantic models with port range and address pattern validation
- âœ… **Resource Management** - Automatic cleanup with server lifespan hooks
- âœ… **Multiple Transports** - Stdio (primary) and HTTP options
- âœ… **Extensive Documentation** - Comprehensive docstrings with examples

### Protocol Support
- ðŸ”Œ **OSC 1.0 Protocol** - Full Open Sound Control specification support
- ðŸ“¡ **UDP Transport** - Low-latency, fire-and-forget messaging
- ðŸ”€ **Multiple Receivers** - Send to multiple applications simultaneously
- ðŸ“¥ **OSC Server** - Receive messages from applications
- ðŸ·ï¸ **Type Support** - Int, float, string, bool values
...
**ðŸŽ›ï¸ `vcv_manager`** - VCV Rack modular synthesis (18+ operations)
...
**ðŸŽ¼ `osc_recorder_manager`** - OSC automation recording (6 operations)
- Record and playback OSC message sequences
- Operations: `start_recording`, `stop_recording`, `playback_recording`, etc.

**ðŸŽµ `music_orchestrator`** - High-level music production workflows (6 operations)
...
**ðŸŽµ `ableton_manager`** - Ableton Live DAW (6 operations)
- Playback control, tempo, clip triggering, mixing
- Operations: `play`, `stop`, `set_tempo`, `play_clip`, `set_volume`, `set_pan`

**ðŸŽ® `vrchat_manager`** - VRChat avatar control (3 basic operations)
- Parameter setting, chat, haptic feedback
- Operations: `set_parameter`, `send_chat`, `trigger_haptic`
- âš ï¸ **Note:** For advanced VRChat features (avatar management, monitoring, complex animations), use the dedicated [vrchat-mcp](https://github.com/sandraschi/vrchat-mcp) repository

**ðŸŽ¨ `touchdesigner_manager`** - TouchDesigner visual programming (40+ operations)
...
**ðŸ”Š `supercollider_manager`** - SuperCollider audio synthesis (3 operations)
- Synth creation, node management, control setting
- Operations: `create_synth`, `free_node`, `set_control`

**ðŸŽ¼ `maxmsp_manager`** - Max/MSP audio/visual programming (3 operations)
- Message sending, DSP control
- Operations: `send_bang`, `send_float`, `toggle_dsp`

**ðŸ“º `resolume_manager`** - Resolume Arena VJ software (3 operations)
- Clip playback, layer control, tempo
- Operations: `play_clip`, `set_layer_opacity`, `set_bpm`

**ðŸŽ›ï¸ `puredata_manager`** - Pure Data visual programming (3 operations)
- Message routing, DSP control
- Operations: `send_bang`, `send_float`, `toggle_dsp`

**ðŸŽµ `audio_workflow_manager`** - Multi-app orchestration (5 operations)
...
## ðŸŽµ Application-Specific Usage
...
osc-mcp/
â”œâ”€â”€ src/oscmcp/
â”‚   â”œâ”€â”€ mcp_server.py        # Primary stdio server (19 tools)
â”‚   â”œâ”€â”€ stdio_server.py      # Alternative stdio server
â”‚   â”œâ”€â”€ server.py            # HTTP server variant
â”‚   â”œâ”€â”€ osc/                 # OSC protocol implementation
â”‚   â”‚   â”œâ”€â”€ client.py        # OSC client for sending
â”‚   â”‚   â””â”€â”€ server.py        # OSC server with message buffering
â”‚   â”œâ”€â”€ apps/                # Application integrations (10+ apps)
â”‚   â”‚   â”œâ”€â”€ ableton.py
â”‚   â”‚   â”œâ”€â”€ touchdesigner.py
â”‚   â”‚   â”œâ”€â”€ vrchat.py
â”‚   â”‚   â””â”€â”€ ... (7 more)
â”‚   â””â”€â”€ midi/                # MIDI integration
â”œâ”€â”€ tests/                   # Test suite
â”œâ”€â”€ docs/                    # Documentation (84+ files)
â”‚   â”œâ”€â”€ APPLICATION_TOOLS_ANALYSIS.md  # Tool analysis
â”‚   â”œâ”€â”€ PROJECT_ANALYSIS.md  # Project maturity assessment
â”‚   â””â”€â”€ adn-notes/           # ADN documentation for each controlee
â”œâ”€â”€ pyproject.toml          # Project configuration
â”œâ”€â”€ UPGRADE_NOTES.md        # FastMCP 3.1.1+ migration guide
...
- **[UPGRADE_NOTES.md](UPGRADE_NOTES.md)** - FastMCP 3.1.1+ â†’ 3.1.1+ migration guide
...
## ðŸ”„ What's New in FastMCP 3.1.1+

OSC-MCP has been upgraded to FastMCP 3.1.1+ with:

âœ… **Server Lifespan Hooks** - Proper resource cleanup on shutdown
âœ… **Response Caching** - 60s TTL for improved performance
âœ… **Pydantic Validation** - Port ranges (1-65535) and address patterns
âœ… **Enhanced Documentation** - 400+ lines of comprehensive docstrings
âœ… **Production Ready** - Resource management and error handling
...
```bash
# Upgrade to FastMCP 3.1.1+.1
pip install --upgrade "fastmcp[all]>=3.1.1+.1"
```
...
- âœ… Server lifespan hooks
- âœ… Response caching
- âœ… Comprehensive docstrings
- âœ… Beta status achieved
- ðŸŽ¯ Gather community feedback
- ðŸŽ¯ Address beta feedback
- ðŸŽ¯ Expand test coverage

### Phase 2: Enhanced Functionality
- âœ… Message buffer with `get_received_messages()` - **Implemented!**
- ðŸŽ¯ Consolidate server implementations
- ðŸŽ¯ OSC connection health monitoring
- ðŸŽ¯ Persistent storage for OSC state
- ðŸŽ¯ Circuit breaker for unreachable hosts
- ðŸŽ¯ Metrics and telemetry

### Phase 3: Application Integration
- âœ… Expose app-specific tools (Ableton, VRChat, etc.) - **19 tools implemented!**
- ðŸŽ¯ Expand tool coverage for existing applications
- ðŸŽ¯ OSCQuery service discovery
- ðŸŽ¯ MIDI bridge tools
...
**Made with â¤ï¸ for the creative technology community**
