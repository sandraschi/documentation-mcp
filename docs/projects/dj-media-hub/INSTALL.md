...
4. Start the server:
   ...
   uv run python -m dj_media_hub.server

   # HTTP mode (for web dashboard)
   uv run uvicorn dj_media_hub.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/dj-media-hub/issues) |
