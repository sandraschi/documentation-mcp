...
4. Start the server:
   ...
   uv run python -m fullstack_demo.server

   # HTTP mode (for web dashboard)
   uv run uvicorn fullstack_demo.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/fullstack-demo/issues) |
