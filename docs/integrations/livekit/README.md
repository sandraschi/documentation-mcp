...
- **teleconference-mcp (Teams++)** - `D:\Dev\repos\teleconference-mcp`
  ...
  - **Extensive project docs:** [docs/projects/teleconference-mcp/](../../docs/projects/teleconference-mcp/) (PRD, STATUS, STRUCTURE, ARCHITECTURE, INTEGRATION_GUIDE)
...
2. **Reference teleconference-mcp**: Clone and run the reference implementation
...
|-----------|------------------|-------|
| **Windows service (fleet)** | `Get-Service LiveKitSFU` | NSSM-wrapped native `livekit-server` 1.7.0, config from `teleconference-mcp/livekit.yaml`, auto-start + crash-restart. Canonical on Goliath |
| Service logs | `Get-Content D:\Dev\repos\teleconference-mcp\logs\livekit.out.log -Tail 50` | Rotated by NSSM |
