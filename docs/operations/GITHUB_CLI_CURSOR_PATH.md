...
## Fleet maintainer heartbeat (PRs & issues)

**Problem:** Small repos get **overlooked PRs**; silence frustrates contributors.

**Tools:** **`github_ops`** — `pr_list` (includes `comments`, `updatedAt` for triage), `issue_list`, `pr_comment`, `issue_comment`. Server: **[git-github-mcp](https://github.com/sandraschi/git-github-mcp)**.

**Human UI:** Web app route **`/inbox`** (fleet multi-repo list; default backend **10702** — [WEBAPP_PORTS.md](WEBAPP_PORTS.md)).

**Schedule:** Not inside the MCP server — use a **supervisor** (robofang, OpenManus, OpenClaw, Hub cron, or GitHub Actions notification). Canonical write-up: **[patterns/GITHUB_MAINTAINER_HEARTBEAT.md](../patterns/GITHUB_MAINTAINER_HEARTBEAT.md)**.
