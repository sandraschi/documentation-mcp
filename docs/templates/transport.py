...
Standard module for MCP servers in d:/Dev/repos.
Provides unified transport configuration for STDIO, HTTP Streamable, and legacy SSE modes.
Per-repo HTTP defaults: see mcp-central-docs/operations/WEBAPP_PORTS.md.
**10700 is virtualization-mcp's slot**, not a generic default—each repo has its own backend port.

Environment Variables:
    MCP_TRANSPORT: Transport mode (stdio, http, sse). Default: stdio
    MCP_HOST: Bind address for HTTP/SSE. Default: 127.0.0.1
    MCP_PORT: Port for HTTP/SSE. Default: 10855 (template placeholder only; replace with this repo's registered backend)
...
ENV_PORT = "MCP_PORT"  # default: 10700 (set per repo to match WEBAPP_PORTS.md backend)
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10855")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10855 — replace with WEBAPP_PORTS backend)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10855

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10855 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument(
        "--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10855)"
    )
    parser.add_argument(
