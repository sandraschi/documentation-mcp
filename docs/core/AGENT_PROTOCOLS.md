...
- **[Unicode Safety](./patterns/unicode_safety.md)**: **MANDATORY** - **EM DASH (`—`) is never allowed** in `start.ps1`, `start.bat`, `justfile`, or other script paths (including comments). Enforce with `mcp-central-docs/scripts/check-unicode-safe.ps1`.
...
**Version History**:
- 1.32 (2026-05-31): **[Git repository safety](./GIT_REPOSITORY_SAFETY.md)** — git+GitHub before batch edits; `ensure-mcp-repo-git.ps1`; checkpoint commits; launcher size guard (`chip-design-mcp` incident).
