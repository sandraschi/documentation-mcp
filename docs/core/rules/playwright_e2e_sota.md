...
### 4. Screenshot Capture for README (developing)
...
| Compose with body → AI panel | Conditional rendering |
...
Enforcement:
- The just e2e recipe (using playwright-audit.ps1) must pass before any repo is marked SOTA
- CI must run npx playwright test on push
- Fleet-wide audit script (playwright-audit.ps1) checks this automatically
...
- Existing repos: Add within 2 weeks of this standard being published
- CI will fail if fewer than 5 e2e tests exist
