...
**Ship NSIS as the primary release asset.** MSI is optional (CI parity); GitHub Releases should list **one** Windows installer, not separate operator/backend binaries.
...
Every Tauri NSIS installer MUST include PREINSTALL and PREUNINSTALL hooks
to kill both the operator and backend processes before install/uninstall.
Without this, the installer hangs when backend.exe is file-locked.

Add POSTINSTALL only if the repo bundles MCP client registration scripts.
...
**Key rules:**
- `KillFleetProcesses` kills BOTH `*-native.exe` and `*-backend.exe` — the backend file-locks the PyInstaller binary.
- The `nsis_tauri_utils::KillProcessCurrentUser` call handles the case where the app runs under the current user session (no SYSTEM token needed).
- `Sleep 2000` gives the OS time to release file handles before NSIS tries to overwrite the files.
- POSTINSTALL is optional — only include if the repo offers MCP client registration.
...
4. Outside Tauri (dev browser), Ctrl+Scroll must still be intercepted and replayed via `setZoom()` -- the browser's native zoom is disabled for this shortcut within the app so the behavior is identical in dev and prod. Browsers that cannot `setZoom()` (not in Tauri) still get the localStorage save so the zoom is remembered when the app runs in Tauri later.
