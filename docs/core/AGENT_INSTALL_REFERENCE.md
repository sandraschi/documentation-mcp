...
| Tauri native installer | `standards/rules/tauri_godot_sota.md` |
...
Fleet SOTA for webapp repos (`standards/rules/tauri_godot_sota.md`):

```
webapp/dist/  +  PyInstaller sidecar  +  Tauri shell (WebView2)  =  NSIS installer
```

**Include:** server code, FastMCP, UI assets, tiny repo-owned bridge files  
