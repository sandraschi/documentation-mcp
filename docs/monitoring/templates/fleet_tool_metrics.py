"""Fleet MCP tool-call metrics (Prometheus). Copy into <package>/fleet_tool_metrics.py.

See mcp-central-docs/monitoring/MCP_MONITORING_STANDARDS.md and QUICK_START.md § Step 3.
"""
...
class McpToolMetricsMiddleware(Middleware):
    """Record per-tool latency and success/error counts for Prometheus."""
...
def register_mcp_tool_metrics(mcp: FastMCP) -> bool:
    """Attach tool-call metrics middleware to a FastMCP server."""
    if not _PROM_AVAILABLE:
        return False
...
def prometheus_metrics_body_and_type() -> tuple[bytes, str]:
    """Body and Content-Type for HTTP ``/metrics`` handlers."""
