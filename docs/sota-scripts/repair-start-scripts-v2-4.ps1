# repair-start-scripts-v2-4.ps1
# Mass repair for fleet start.ps1 files using webapp-registry.json for ports
# V2.4: Robust registry lookup and refined entry point logic
...
foreach ($Script in $StartScripts) {
    ...
    if ($Content -match "npm run dev" -or $Content -match "uvicorn" -or $Content -match "python -m" -or $Content -match "Auto-Repaired") {
        ...
        # 2. Look up ports in registry (Case-Insensitive lookup)
        $MatchedApps = $Registry.webapps | Where-Object { 
        ...
        elseif ($MatchedApps.Count -ge 1) {
            $WebPort = $MatchedApps[0].port
            Write-Host "  Found Port via Registry: $WebPort" -ForegroundColor DarkGreen
        }
        ...
        elseif ($ModuleName -eq "virtualization_mcp") { $EntryPoint = "virtualization_mcp.server:app" } # Explicit
        ...
        $SOTATemplate = @"
# Webapp Start - Standardized SOTA (Auto-Repaired V2.4)
`$WebPort = $WebPort
...
Write-Host "Mass repair V2.4 complete." -ForegroundColor Green
