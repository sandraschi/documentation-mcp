# repair-start-scripts-v2-3.ps1
# Mass repair for fleet start.ps1 files using webapp-registry.json for ports
# V2.3: Fixed PowerShell escaping for background processes using confirmed logic
...
foreach ($Script in $StartScripts) {
    ...
    if ($Content -match "npm run dev" -or $Content -match "uvicorn" -or $Content -match "python -m" -or $Content -match "Auto-Repaired") {
        ...
        elseif ($Content -match "web_bridge:app") { $EntryPoint = $ModuleName + ".web_bridge:app" } # Simplified for src layout
        elseif ($Content -match "webapp.backend.main:app") { $EntryPoint = "webapp.backend.main:app" }
        elseif ($Content -match "handbrake_mcp.main:app") { $EntryPoint = "handbrake_mcp.main:app" }
        elseif ($Content -match "virtualdj_mcp.main:app") { $EntryPoint = "virtualdj_mcp.main:app" }
        
        # Refine EntryPoint for src layout if needed
        if (Test-Path (Join-Path $SrcDir $ModuleName "server.py")) {
            $EntryPoint = "${ModuleName}.server:app"
        }
        ...
        $SOTATemplate = @"
# Webapp Start - Standardized SOTA (Auto-Repaired V2.3)
`$WebPort = $WebPort
...
# Use triple-backtick to ensure `$env:PYTHONPATH reaches the background shell literally as `$env:PYTHONPATH
...
Write-Host "Mass repair V2.3 complete." -ForegroundColor Green
