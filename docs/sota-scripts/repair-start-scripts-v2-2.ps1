# repair-start-scripts-v2-2.ps1
# Mass repair for fleet start.ps1 files using webapp-registry.json for ports
# V2.2: Fixed PowerShell escaping for background processes
...
foreach ($Script in $StartScripts) {
    ...
    if ($Content -match "npm run dev" -or $Content -match "uvicorn" -or $Content -match "python -m") {
        ...
        $SOTATemplate = @"
# Webapp Start - Standardized SOTA (Auto-Repaired V2.2)
`$WebPort = $WebPort
...
# Use double backtick to ensure `$env:PYTHONPATH reaches the background shell literally
...
Write-Host "Mass repair V2.2 complete." -ForegroundColor Green
