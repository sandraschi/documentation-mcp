# SOTA Startup Propagation Tool: Recursive Discovery Edition
...
foreach ($row in $report) {
    ...
    if (Test-Path $srcPath) {
        ...
        foreach ($f in $foundFiles) {
            ...
            if ($fContent -match "FastMCP|FastAPI|uvicorn") {
                # Convert absolute path to module path relative to src
    ...
    # Fallback to non-src if still null
    if (-not $bEntry) {
        $foundFiles = Get-ChildItem -Path $projectRootPath -Filter "*.py" -MaxDepth 2 | Where-Object { 
            $_.Name -match "^(http_server|main|server|app)\.py$" 
        }
        foreach ($f in $foundFiles) {
            $fContent = Get-Content $f.FullName -Raw
    ...
    Write-Host "Re-Fixing $($row.Repo)... (Backend: $($bEntry ?? 'None'))"
