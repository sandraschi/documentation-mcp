...
const BACKEND_NAME: &str = "documentation-mcp-backend.exe";
const BACKEND_PORT: u16 = 11033;
const BACKEND_TAG: &str = "documentation-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "PORT";
const ENV_HOST: &str = "HOST";
const ENV_TAURI: &str = "DOCUMENTATION_MCP_TAURI";
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name documentation-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM documentation-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
